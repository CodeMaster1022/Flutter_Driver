  ...
  Future<void> onNextButtonPressed(BuildContext context) async {
    ...
    if (widget.loginMethod == 'email') {
      await ApiService.loginDriver(
          driverViewModel.driver.email!, passwordController.text, "email");
    } else {
      await ApiService.loginDriver(driverViewModel.driver.phoneNumber!,
          passwordController.text, "phone");
    }

    if (driverViewModel.isLoginSuccessful) {
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => RegisterDonePage()),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(driverViewModel.errorMessage ?? 'Login failed')),
      );
    }
  }
